      ...
      const response = await fetch(MISTRAL_API_ENDPOINT, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: `You are an AI assistant. Here is information that you can use to answer questions:
              ${JSON.stringify(personalInfo, null, 2)}
              Instructions:
              1. Be friendly and conversational
              2. Format responses with markdown for better readability
              3. Keep responses concise but informative
              4. If asked about technical projects, provide specific details
              5. For personal questions, maintain a professional tone
              6. Use bullet points or numbered lists when appropriate
              7. Include relevant context from multiple categories when applicable`
            },
            {
              role: "user",
              content: question
    ...
    try {
      ...
      const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        headers: {
          ...
          'X-Title': 'My AI Assistant' // Site name for rankings
        },
        body: JSON.stringify({
